...
import {
  ...
  calculatePersonalizedGoalForDate,
  saveDailyGoal,
  getDailyGoal,
  logout,
  isAuthenticated,
...
export default function Settings() {
  ...
  const [progressWheelStyle, setProgressWheelStyle] =
    useState<ProgressWheelStyle>("drink-colors");
  ...
  // Edit drink state
  const [editingDrink, setEditingDrink] = useState<CustomDrinkType | null>(
    null
  );
  const [isEditDrinkDialogOpen, setIsEditDrinkDialogOpen] = useState(false);
  const [isExportingData, setIsExportingData] = useState(false);
  const [isDeletingAccount, setIsDeletingAccount] = useState(false);
  const [healthConnectionCode, setHealthConnectionCode] = useState("");
          ...
          try {
            ...
            if (userId) {
              ...
              if (response?.user) {
                ...
                const backendWeightUnit =
                  (backendUser.weightUnit as WeightUnit) || "kg";
                const weightInPreferredUnit = backendUser.weight;
                const weightInKg =
                  backendWeightUnit === "lbs"
                ...
                const backendUnit =
                  volumeUnitMap[backendUser.volumeUnit] || "ml";
                setUnitPreference(backendUnit);
                saveUnitPreference(backendUnit);

                const tempUnit =
                  backendUser.temperatureUnit === "fahrenheit" ? "F" : "C";
                ...
                const backendStyle =
                  wheelStyleMap[backendUser.progressWheelStyle] ||
                  "drink-colors";
                ...
                const hasCustomGoal =
                  backendUser.customGoalLiters != null &&
                  backendUser.customGoalLiters > 0;
                const goalLiters =
                  hasCustomGoal && backendUser.customGoalLiters
                ...
                // Check Archer Health connection
                if (backendUser.archerHealthConnectionCode) {
                  setHealthConnected(true);
                  // Perform health check in background
                  performHealthCheck(backendUser.archerHealthConnectionCode);
                }
            ...
            console.warn(
              "Failed to load user data from backend, using local storage",
              error
            );
          }
        }
  ...
  const handleUpdateProfile = async () => {
    ...
    const weightInKg =
      weightUnit === "lbs" ? lbsToKg(weightValue) : weightValue;

    if (weightInKg < 30 || weightInKg > 300) {
      toast.error(
        `Please enter a valid weight (${
          weightUnit === "lbs" ? "66-660 lbs" : "30-300 kg"
        })`
      );
      return;
    }
    ...
    if (backendIsEnabled()) {
      try {
        ...
        toast.warning(
          "Profile updated locally, but failed to sync with server"
        );
      }
    } else {
      toast.success("Profile updated!");
  ...
  const handleRecalculateGoal = async () => {
    ...
    const personalizedGoalMl = calculatePersonalizedGoal(
      profile,
  ...
  const handleUpdateManualGoal = async () => {
    ...
    if (backendIsEnabled() && isAuthenticated()) {
      try {
        ...
        toast.warning(
          "Custom goal saved locally, but failed to sync with server"
        );
        return;
      }
    }
  ...
  const handleExportUserData = async () => {
    if (!backendAvailable) {
      toast.info(
        "Connect to the backend service to export your hydration data."
      );
      return;
    }
  ...
  const handleDeleteAccount = async () => {
    ...
    const confirmed = window.confirm(
      "This will permanently delete your account and hydration history. This action cannot be undone."
    );
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to delete your account"
      );
    } finally {
  ...
  const performHealthCheck = async (connectionCode: string) => {
    try {
      const healthUrl =
        import.meta.env.VITE_ARCHER_HEALTH_URL || "https://health.adarcher.app";
      const resp = await fetch(`${healthUrl}/api/hydration-goals`, {
        headers: {
          Authorization: `Bearer ${connectionCode}`,
        },
      });
      if (!resp.ok) {
        // If sync fails, check if it's because user doesn't exist
        if (resp.status === 401 || resp.status === 404) {
          console.warn("Archer Health user not found, resetting connection");
          setHealthConnected(false);
          setHealthError(
            "Linked Archer Health account no longer exists. Connection reset."
          );
          // Optionally clear the connection code from backend
          // But for now, just update UI
        } else {
          setHealthError(`Health sync failed (${resp.status})`);
        }
      } else {
        setHealthError(null);
      }
      ...
      setHealthError("Unable to connect to Archer Health");
    }
  };

  const handleDisconnectArcherHealth = async () => {
    try {
      ...
      const response = await fetch("/api/auth/connection-code", {
        method: "DELETE",
        headers,
      });

      if (response.ok) {
        toast.success("Disconnected from Archer Health");
        setHealthConnected(false);
        setHealthError(null);
      } else {
      ...
      console.error("Failed to disconnect from Archer Health", error);
      toast.error("Network error. Please try again.");
    }
  };

  const handleConnectArcherHealth = async () => {
    if (!healthConnectionCode.trim()) {
    ...
    try {
      ...
      const response = await fetch("/api/auth/redeem-connection-code", {
        method: "POST",
        headers,
        body: JSON.stringify({ connectionCode: healthConnectionCode.trim() }),
      });

      if (response.ok) {
        toast.success("Successfully connected to Archer Health!");
        setHealthConnectionCode("");
        setHealthError(null);

        // Refresh user data to get the updated connection code
        try {
          const authState = await getAuthState();
          ...
          // Still show as connected since the API call succeeded
          setHealthConnected(true);
        }
      } else {
        const data = await response.json();
        toast.error(data.error || "Failed to connect");
        setHealthError(data.error || "Failed to connect");
      }
    } catch (error) {
      console.error("Failed to connect to Archer Health", error);
      toast.error("Network error. Please try again.");
    }
  };
  ...
  const handleSyncWithHealth = async () => {
    ...
    try {
      ...
      // Get auth state to access connection code
      const authState = await getAuthState();
      if (!authState?.user?.archerHealthConnectionCode) {
        setHealthError("No connection to Archer Health found");
        toast.error("No connection to Archer Health found");
        return;
      }

      // Sync hydration goals from Archer Health
      try {
        const goalsResp = await fetch(`${healthUrl}/api/hydration-goals`, {
          headers: {
        ...
        if (goalsResp.ok) {
          ...
          if (goalsData.waterGoal) {
            // Convert units if needed
            let goalInMl = goalsData.waterGoal;
            if (goalsData.waterGoalUnit === "oz") {
              goalInMl = goalsData.waterGoal * 29.5735; // Convert oz to ml
            }

            // Update local goal
            saveDailyGoal(goalInMl);

            // Sync to Aqua backend if enabled
            if (backendIsEnabled()) {
              await setDailyGoalToBackend(getTodayKey(), goalInMl);
            }
          }
        } else {
          console.warn(
            "Failed to fetch goals from Archer Health:",
            goalsResp.status
          );
        }
      } catch (goalsError) {
        console.warn(
          "Failed to sync hydration goals from Archer Health:",
          goalsError
        );
        // Don't fail the whole sync if goals sync fails
      }

      toast.success("Synced with Archer Health");
    } catch (error) {
      console.error("Health sync error", error);
      setHealthError("Network error while contacting Archer Health");
      toast.error("Could not reach Archer Health. Try again later.");
    }
  };
  ...
  const handleTemperatureUnitChange = async (unit: TemperatureUnit) => {
    ...
    if (backendIsEnabled()) {
      try {
        ...
        console.error(
          "Failed to sync temperature preference to backend",
          error
        );
      }
    }
  ...
  const handleAddCustomDrink = async () => {
    ...
    try {
      ...
      toast.error(
        `Failed to create "${newDrink.name}": ${
  ...
  const handleUpdateCustomDrink = async () => {
    ...
    try {
      ...
      const updatedDrinks = customDrinks.map((d) =>
        d.id === editingDrink.id ? editingDrink : d
      );
      setCustomDrinks(updatedDrinks);
      setCustomDrinksList(updatedDrinks);
      ...
      toast.error(
        `Failed to update "${editingDrink.name}": ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
      return;
  ...
  const handleRemovePassword = async () => {
    if (!isGoogleUser) {
      toast.error(
        "Cannot remove password without alternative authentication method"
      );
      return;
    }
  ...
  const handleWeatherAdjustmentChange = async (enabled: boolean) => {
    ...
    if (backendIsEnabled()) {
      try {
        ...
        if (userId) {
          await updateBackendUser(userId, {
            weatherAdjustmentsEnabled: enabled,
          });
        }
      } catch (error) {
        console.error(
          "Failed to sync weather adjustment preference to backend",
          error
        );
      }
    }
    ...
    toast.success(
      `Weather-based adjustment ${enabled ? "enabled" : "disabled"}`
    );
  };
    ...
    if (profile && usePersonalizedGoal) {
      ...
      if (useWeatherAdjustment) {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          if (dayRecord) {
            const adjustedGoal = calculatePersonalizedGoalForDate(
              profile,
              dateKey,
              true
            );
          ...
          <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {isGoogleUser ? (
                      <p className="text-xs text-blue-600">
                        Linked to Google account - unlink to change email
                      </p>
                    ) : emailVerified ? (
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="text-sm space-y-1">
                      {isGoogleUser && (
                        <p className="text-blue-600">✓ Google Sign-in</p>
                      )}
                      {hasPassword ? (
                  ...
                  <Button
                    ...
                    className="flex-1 mt-2 md:col-span-2"
                  >
                    Update Profile
                  </Button>
                </div>
              ...
              <CardContent className="space-y-4">
                ...
                {!useWeatherAdjustment && (
                  <div className="space-y-2">
                    ...
                    <Select
                      value={climate}
                      onValueChange={(value: "cold" | "moderate" | "hot") =>
                        setClimate(value)
                      }
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="cold">
                          Cold (below{" "}
                          {temperatureUnit === "F" ? "59°F" : "15°C"})
                        </SelectItem>
                        <SelectItem value="moderate">
                          Moderate (
                          {temperatureUnit === "F" ? "59-77°F" : "15-25°C"})
                        </SelectItem>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Enable weather-based adjustments below for automatic
                      climate detection
                    </p>
                  </div>
                )}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Set a manual daily water goal or use our personalized
                  recommendation
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <Button
                  onClick={handleUpdateManualGoal}
                  variant="outline"
                  className="w-full"
                >
                  Set Goal
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    value={progressWheelStyle}
                    onValueChange={(value: ProgressWheelStyle) =>
                      handleProgressWheelStyleChange(value)
                    }
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="drink-colors">
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            Drink Colors (Default - Reflects what you drink)
                          </span>
                        </div>
                      </SelectItem>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Dialog
                    open={isAddDrinkDialogOpen}
                    onOpenChange={setIsAddDrinkDialogOpen}
                  >
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Create a custom drink type with your own hydration
                          multiplier
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2">
                          <Label>
                            ...
                            {newDrinkMultiplier[0].toFixed(2)}x
                          </Label>
                          <Slider
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {drink.hydrationMultiplier.toFixed(2)}x
                                hydration
                              </p>
                            </div>
                          </div>
            ...
            <Dialog
              open={isEditDrinkDialogOpen}
              onOpenChange={setIsEditDrinkDialogOpen}
            >
              <DialogContent>
                <DialogHeader>
                ...
                {editingDrink && (
                  <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      <Label>
                        Hydration Multiplier:{" "}
                        {editingDrink.hydrationMultiplier.toFixed(2)}x
                      </Label>
                      <Slider
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        {editingDrink.hydrationMultiplier < 0
                          ? "Dehydrating"
                          : editingDrink.hydrationMultiplier < 0.5
                          ? "Low hydration"
                          : editingDrink.hydrationMultiplier < 0.9
                          ? "Moderate hydration"
                          : editingDrink.hydrationMultiplier < 1.1
                          ? "Full hydration"
                          : "Enhanced hydration"}
                      </p>
                    </div>
                    ...
                    <Button
                      onClick={handleUpdateCustomDrink}
                      className="w-full bg-gradient-water"
                    >
                      Update Custom Drink
                    </Button>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Set your timezone for accurate daily tracking across different
                  locations
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Real-time weather conditions affecting your hydration
                      needs
                    </p>
            ...
            {/* Archer Health Connection */}
            <Card>
              <CardHeader>
                <CardTitle>Archer Health Integration</CardTitle>
                <CardDescription>
                  Connect your Archer Health account to sync hydration goals and
                ...
                {healthConnected ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        onClick={handleDisconnectArcherHealth}
                        variant="outline"
                        className="flex-1"
                      >
                        Disconnect
                      ...
                      <Button
                        onClick={() => {
                          // open health app in new window
                          const url =
                            import.meta.env.VITE_ARCHER_HEALTH_URL ||
                  ...
                  <>
                    <div className="space-y-2">
                      <Label htmlFor="health-code">Connection Code</Label>
                      <Input
                        id="health-code"
                        value={healthConnectionCode}
                        onChange={(e) =>
                          setHealthConnectionCode(e.target.value)
                        }
                        placeholder="Enter code from Archer Health"
                      />
                    </div>
                    <p className="text-sm text-amber-600 dark:text-amber-400">
                      <AlertTriangle className="h-4 w-4 text-amber-500 inline mr-1" />
                      Warning: Connecting your accounts links them permanently.
                      You cannot unlink accounts unless one account is deleted.
                    </p>
                    <Button
                      onClick={handleConnectArcherHealth}
                      variant="outline"
                      className="w-full"
                    >
                      Connect to Archer Health
                    </Button>
                    <p className="text-xs text-muted-foreground">
                      Visit{" "}
                      <a
                      ...
                      <button
                        ...
                        className="ml-2 inline-flex items-center px-2 py-1 border rounded text-sm text-primary hover:bg-primary/5"
                      >
                        Open Archer Health
                      </button>
                    </p>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {backendAvailable ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    Connect to the Archer Aqua backend to export or delete your
                    synced account.
                  </p>
                )}
                <Button
